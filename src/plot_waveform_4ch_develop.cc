...
constexpr int TIME_TO_SLEEP = 300;
//std::thie_thread::sleep_for( std::chrono::milliseconds(TIME_TO_SLEEP) )

void plot_waveform_4ch_develop(std::string fileNum, enum runMode opt)
{
  int ch_to_plot;
  FILE *fp;
  int file_size;
  ...
  TCanvas *c1 = new TCanvas("c1", "Waveform", 1000, 1000);
  c1->Divide(2, 2);
  TH1F *plot1 = new TH1F("plot1", (TString)( "Waveform ch"+std::to_string(chInput.at(0) + 1) ), 1000, 0, 200); 
  TH1F *plot2 = new TH1F("plot2", (TString)( "Waveform ch"+std::to_string(chInput.at(1) + 1) ), 1000, 0, 200); 
  TH1F *plot3 = new TH1F("plot3", (TString)( "Waveform ch"+std::to_string(chInput.at(2) + 1) ), 1000, 0, 200); 
  TH1F *plot4 = new TH1F("plot4", (TString)( "Waveform ch"+std::to_string(chInput.at(3) + 1) ), 1000, 0, 200); 
  ...
  std::cout << "Total event size = " << std::to_string(nevt) << std::endl;
  ...
  if (opt == runMode::kPrompt){
    ...
    while (repeat) {
      ...
      scanf("%d", &evtNum);
      if (evtNum > 0 && evtNum < nevt) continue_evtNum = evtNum;
      if (evtNum >= nevt) {
        std::cout << "Total event in this file is : " << std::to_string(nevt) << std::endl;
        std::cout << "Please enter valid event number" << std::endl;
        continue;
      }
      fp = fopen((DATA_DIR+FILE_NAME+"_"+fileNum+".dat").c_str(), "rb");
      //Draw plot
      for (int evt = 0; evt < nevt; ++evt){
        ...
        plot1->SetTitle((TString)( "Waveform ch"+std::to_string(chInput.at(0) + 1) + " Evt num = " + std::to_string(evt)));
  ...
  if (opt == runMode::kFull) {
    ...
    for (int evt = 0; evt < nevt; evt++) {
      ...
      if (continue_evt == true) {
        std::cout << "Continue reading events from evt num : " <<std::to_string(continue_evtNum) << std::endl;
        if (!(evt == continue_evtNum)) continue;
        else continue_evt = false;
      }
